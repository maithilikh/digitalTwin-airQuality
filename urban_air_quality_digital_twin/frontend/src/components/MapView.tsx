...
const MapView: React.FC<MapViewProps> = ({ markerData, onMapClick }) => {
  const defaultPosition: [number, number] = [12.977, 77.589]; // Bangalore
  return (
    <MapContainer
      {...({ center: defaultPosition, zoom: 12 } as any)}
      ...
      {markerData && (
        <Marker position={[markerData.lat, markerData.lng]}>
          <Popup>
            <div
              style={{
                borderLeft: `6px solid ${getAQIColor(markerData.category)}`,
                paddingLeft: 8,
              }}
            >
              <div>
                <strong>{markerData.city}</strong>
              </div>
              <div>
                AQI: <b>{markerData.aqi}</b>
              </div>
              <div>
                Category:{" "}
                <span style={{ color: getAQIColor(markerData.category) }}>
                  {markerData.category}
                </span>
              </div>
